...
class Backbone(BackboneBase):
    ...
    def __init__(self,
        ...
        if 'vit' in name:
            if name == 'vit_tiny':
                embed_dim, depth, num_heads, dp = 192, 12, 12, 0.
            elif name == 'vit_small':
                embed_dim, depth, num_heads, dp = 384, 12, 12, 0.
            elif name == 'vit_base':
                embed_dim, depth, num_heads, dp = 768, 12, 12, 0.
            else:
                raise NotImplementedError("Backbone {} currently not supported.".format(name))
            ...
            self.encoder = SparseViT(  # Single-scale ViT encoder
    ...
    def forward(self, tensor_list: NestedTensor, sparsity_mask=None):
        ...
        """
        # (H, W, B, C)
        feats = self.encoder(tensor_list.tensors, sparsity_mask)
    ...
    def forward_export(self, tensors:torch.Tensor, sparsity_mask=None):
        feats = self.encoder(tensors, sparsity_mask)
        feats = self.projector(feats)
        out_feats = []
        out_masks = []
        for feat in feats:
