...
from util.timers import TimerDummy as CudaTimer
import time
...
class Block(nn.Module):
    """Transformer blocks with residual propagation blocks"""
...
class SparseViT(nn.Module):
    ...
    def forward(self, x, sparsity_mask):
        with CudaTimer(timer_name="Patch Embed"):
        ...
        h, w = H // 4, W // 4
        x = x.reshape(B, 4, h, 4, w, C).permute(0, 1, 3, 2, 4, 5)
        
        with CudaTimer(timer_name="Filtering"):
            # No windows required, reshape in non-window representation
            x = x.reshape(B, H*W, C)

            max_true_count = int(sparsity_mask.sum(dim=1).max())

            if B > 1:
                attention_mask = torch.ones((B, max_true_count), dtype=torch.bool, device=x.device)
            else:
                attention_mask = None
            x_filtered = torch.zeros((B, max_true_count, C), dtype=x.dtype, device=x.device)

            true_indices_save = []
            num_true_indices_save = []
            for i in range(B):
                true_indices = sparsity_mask[i].nonzero(as_tuple=True)[0]
                num_true_indices = true_indices.shape[0]
                x_filtered[i, :num_true_indices, :] = x[i, true_indices, :]
                if B > 1:
                    attention_mask[i, :num_true_indices] = False

                true_indices_save.append(true_indices)
                num_true_indices_save.append(num_true_indices)
        
        out = []
        for idx, blk in enumerate(self.blocks):
            with CudaTimer(timer_name="Attention"):
                x_filtered = blk(x_filtered, mask=attention_mask)

            if self._out_features[idx]:
                with CudaTimer(timer_name="Filter return"):

                    for i in range(B):
                            x[i, true_indices_save[i], :] = x_filtered[i, :num_true_indices_save[i], :]
