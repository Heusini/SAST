...
class LWDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, sparsity_mask, targets=None):
        ...
        with CudaTimer(timer_name="Transformer"):
            hs, ref_unsigmoid, hs_enc, ref_enc = self.transformer(
                srcs, masks, poss, refpoint_embed_weight, query_feat_weight)
            
        with CudaTimer(timer_name="Class and BBox embedding"):
    ...
    def forward_export(self, tensors, sparsity_mask, targets=None):
        with CudaTimer(timer_name="Backbone"):
            srcs, _, poss = self.backbone(tensors, sparsity_mask)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.ia_bce_loss:
            ...
            pos_weights = torch.zeros_like(src_logits).float()
...
class PostProcess(nn.Module):
    ...
    def __init__(self, num_select=300, height=360, width=640) -> None:
        super().__init__()
    ...
    def forward(self, outputs):
        ...
        scale_fct = torch.tensor([self.width, self.height, self.width, self.height]).to(boxes.device)
...
def build(args):
    ...
    # if args.dataset_file == 'armasuisse' or args.dataset_file == 'armasuisse_split':
    #     num_classes = 1

    # if args.dataset_file == 'prophesee':
    #     num_classes = 3
