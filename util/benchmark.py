...
def benchmark(model, dataset, output_dir):
    ...
    for idx in range(total_step):
        data, _, sparsity_mask = dataset[idx]

        # Adjust for sparsity amount
        #sparsity_mask[:] = False
        #sparsity_mask[:] = True
        data_tensor.append(([data.cuda().unsqueeze(0)], [sparsity_mask.cuda().unsqueeze(0)]))

    # import pdb; pdb.set_trace()
    with torch.no_grad():
        tmp = []
        tmp2 = []
        for id, data in enumerate(tqdm.tqdm(data_tensor)):
            res = flop_count(model, data)
            t = measure_time(model, data)
