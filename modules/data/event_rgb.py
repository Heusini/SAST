...
class EventRGBDataModule(pl.LightningDataModule):
    def __init__(self, 
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        use_percent_dataset = 1
        if stage == 'fit':
            if self.train_sampling_mode in (DatasetSamplingMode.RANDOM, DatasetSamplingMode.MIXED):

                event_rgb_dataset = EventRGBDataset.build(dataset_mode=DatasetMode.TRAIN,
                                                          dataset_config=self.dataset_config)
                partial_dataset = PartialDataset(event_rgb_dataset, use_percent_dataset)
                augmented_dataset = AugmentedDataset.build(dataset_config=self.dataset_config, 
                                                        dataset=partial_dataset)


                self.sampling_mode_2_dataset[DatasetSamplingMode.RANDOM] = augmented_dataset


            event_rgb_dataset = EventRGBDataset.build(dataset_mode= DatasetMode.VALIDATION,
                                                      dataset_config=self.dataset_config)
            partial_dataset = PartialDataset(event_rgb_dataset, use_percent_dataset)
